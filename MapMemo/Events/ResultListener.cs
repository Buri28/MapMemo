...
namespace MapMemo.Domain
{
    public class ResultListener : IInitializable, IDisposable
    {
        ...
        {
            if (Plugin.VerboseLogs) Plugin.Log.Info("ResultListener Initialized");
            // イベント購読開始
            ...
            if (type == null) return;
            if (Plugin.VerboseLogs) Plugin.Log.Info("ResultListener: Found BeatLeader LeaderboardEvents type");
            ...
            if (eventInfo != null)
            {
                if (Plugin.VerboseLogs) Plugin.Log.Info("ResultListener: Subscribing to BeatLeader ScoreStatsRequestedEvent");
                ...
                if (Plugin.VerboseLogs) Plugin.Log.Info($"ResultListener: Event handler signature: {string.Join(", ", invoke.GetParameters().Select(p => p.ParameterType.Name))}");
                try
                {
                    ...
                    if (Plugin.VerboseLogs) Plugin.Log.Info("ResultListener: Subscribed successfully");
                }
                catch (Exception ex)
                {
                    if (Plugin.VerboseLogs) Plugin.Log.Error($"ResultListener: Failed subscribe: {ex}");
                }
            }
        }
        ...
        {
            if (Plugin.VerboseLogs) Plugin.Log.Info($"ResultListener Event Fired! ID: {someId}");
        }

        public void Dispose()
        {
            if (Plugin.VerboseLogs) Plugin.Log.Info("ResultListener Disposed");
            // メモリリーク防止のため解除
            _sceneTransition.didFinishEvent -= OnLevelFinished;
        }
